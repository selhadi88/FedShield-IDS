...
KERNEL_SIZE = 3   # run this script 3 times total: 3, then 5, then 7
# =============================================================

DATA_DIR = '/content/drive/MyDrive/FedShield_data'
CKPT_DIR = f'{DATA_DIR}/checkpoints_k{KERNEL_SIZE}'
...
# ---- Rebuild the identical held-out test set (same seed -> same test set every time) ----
...
if ckpt_files:
    ...
    print(f"Resuming k={KERNEL_SIZE} from checkpoint: {last} (round {start_round} already complete)")
...
for rnd in range(start_round, R):
    ...
    print(f"k={KERNEL_SIZE} Round {rnd+1}/{R} done and checkpointed")
...
print(f"\n=== SUMMARY FOR TABLE (k={KERNEL_SIZE}) ===")
