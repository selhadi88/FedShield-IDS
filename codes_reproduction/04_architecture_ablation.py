...
ARCH = "cnn_only"   # run this script twice: "cnn_only", then "lstm_only"
# =============================================================

DATA_DIR = '/content/drive/MyDrive/FedShield_data'
CKPT_DIR = f'{DATA_DIR}/checkpoints_{ARCH}'
...
def build_model(arch=ARCH):
    ...
    else:  # full hybrid, for sanity-check only
        x = layers.Conv1D(64, 1, padding='same', activation='relu')(inp)
        x = layers.MaxPool1D()(x)
        x = layers.LSTM(64)(x)
...
if ckpt_files:
    ...
    print(f"Resuming {ARCH} from checkpoint: {last} (round {start_round} already complete)")
...
for rnd in range(start_round, R):
    ...
    print(f"{ARCH} Round {rnd+1}/{R} done and checkpointed")
